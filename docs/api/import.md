...
将旧版本软件录制的 `.acctlm` 文件解析为 [`RecordingOutcome`](recording.md#recordingoutcome)，
供主模块（m1）在导入历史录制时使用。
...
**参数**：
- `path` — `.acctlm` 文件路径。可以是 `&str`、`String`、`Path`、`PathBuf` 等实现 `AsRef<Path>` 的类型。
...
| Magic 字节 | `b"ACTL\r\n\x1A\n"` 不匹配 | `TelemetryError::InvalidFormat` |
...
| `laps` | 从 Lap Index block 读取，`lap_time` 由 tick 差值估算；`split_times` 恒为空 |

---

## 5. 圈速说明

- `LapSummary.lap_time` 由 `(end_tick - start_tick) / poll_hz` 计算，精度取决于录制帧率。
- `LapSummary.split_times` 在导入场景下始终为空。扇区级别的分段时间未存储在 Lap Index 中。
- `LapSummary.is_valid` 直接对应 ACC 判定（`is_valid != 0`）。
- Out lap（`is_out_lap != 0`）的 `lap_time` 为 `None`（start_tick == end_tick）。
...
match parse_acctlm_file("unknown.acctlm") {
    Ok(outcome) => {
...
- [按圈提取遥测数据 (`extract_lap_telemetry`)](extract-lap-telemetry.md) — 解析文件后按圈提取指定 raw 字段的逐帧数据
```
