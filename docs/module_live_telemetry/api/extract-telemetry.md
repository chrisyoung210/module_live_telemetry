...
> **相关文档**: [import.md](import.md) · [raw-item.md](raw-item.md) · [recording.md](recording.md)

将 `.acctlm2` 或旧版 `.acctlm` 文件中指定 raw 字段的遥测数据按圈组织提取，供主模块（m1）在数据分析、可视化场景中使用。
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 2. 调用 API
    let laps = extract_lap_telemetry("recording_20260606.acctlm2", &keys)?;
...
| 参数 | 类型 | 说明 |
|---|---|---|
| `path` | `impl AsRef<Path>` | `.acctlm2` 或 `.acctlm` 文件路径。支持 `&str`、`String`、`Path`、`PathBuf`。 |
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 3. 调用 API
    let laps = extract_laps_telemetry("recording_20260606.acctlm2", &keys, lap_numbers)?;
...
- [acctlm/acctlm2 文件导入 (`parse_acctlm_file`)](import.md) — 解析文件摘要信息（赛道、车型、每圈圈时）
