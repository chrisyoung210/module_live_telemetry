...
> ⚠️ **本文档已过期 — 仅供参考历史实现。**
> 
> 本文档描述的计算逻辑基于旧的 CLI 代码路径 (`src/bin/acc-live-telemetry.rs`)。
> 当前项目已迁移到新的计算架构：
> - **逐帧计算**: `src/compute/items.rs` — `RealtimeComputeItem` trait + `ComputeRegistry`
> - **批量计算**: `src/compute/items.rs` — `BatchComputeItem` trait
> - **圈检测**: `src/recording/engine.rs` — `LapCompletedEvent` + recording engine
> - **Dashboard 调度**: `src/dashboard/service.rs` — `DashboardService`
> 
> **当前文档请参考**: [calculated-item.md](../api/calculated-item.md) · [sector-calculated-items.md](sector-calculated-items.md) · [recording.md](../api/recording.md)

本文档说明旧代码中实际存在的遥测数据计算逻辑。所有计算均在 `src/bin/acc-live-telemetry.rs` 中完成。
