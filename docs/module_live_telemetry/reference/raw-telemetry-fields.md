...
> **相关文档**: [api/raw-item.md](../api/raw-item.md) · [format/v2-acctlm2-format-spec.md](../format/v2-acctlm2-format-spec.md)
> 
...
| `rawStaticPage` | header `stat_sz` | full page fields | full static page from `Local\acpmf_static` / 当前实现保存完整静态页 |
...
The current implementation stores the full static shared-memory page. The exact
byte length is written in the `.accraw` header as `stat_sz`, so parsers should
use the header value instead of assuming a fixed 200-byte prefix.
