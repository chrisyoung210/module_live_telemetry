# acctlm2 Binary Telemetry Format Specification
...
For each group:
  ...
  For each column:
    [u16] column_id    (1–213)
    [u8]  value_type   (1=U64, 2=I32, 3=F32, 4=F64, 5=BYTES)
    [u8]  name_len
...
`abs_physics` (F32), `engine_brake` (F32), `ers_recovery_level` (F32),
`ers_power_level` (F32), `ers_heat_charging` (F32), `ers_is_charging` (F32),
`drs_available` (F32), `drs_enabled` (F32), `tc_in_action` (F32),
`abs_in_action` (F32), `auto_shifter_on` (F32), `current_max_rpm` (F32),
`p2p_activations` (F32), `p2p_status` (F32), `water_temp` (F32).

### 3.3 Group 2 — VehicleDynamics (7 columns)

Vehicle motion (v1's `Motion` cluster minus `sample_tick`/`timestamp_ns`).

| 20 | `velocity` | BYTES (3 × f64) |
| 21 | `acc_g` | BYTES (3 × f64) |
| 22 | `local_velocity` | BYTES (3 × f64) |
| 23 | `local_angular_vel` | BYTES (3 × f64) |
| 24 | `heading` | F32 |
| 25 | `pitch` | F32 |
| 26 | `roll` | F32 |
...
encoded as `TYPE_BYTES` with `sub_value_count=4`.
...
`wind_direction`, `surface_grip`, `rain_intensity`, `rain_intensity_in_10min`,
`rain_intensity_in_30min` (all F32).
...
(33-wide BYTES array), MFD settings, `tc_level`, `tc_cut`, `engine_map`,
`abs_level`, `exhaust_temperature`, vibration fields, and 3-wide BYTES arrays
for `car_coordinates` and `car_id`.
...
| `value_type` | 1 | `0x01`=U64, `0x02`=I32, `0x03`=F32, `0x04`=F64, `0x05`=BYTES |
...
> In v2: `TYPE_U64=1, TYPE_I32=2, TYPE_F32=3, TYPE_F64=4, TYPE_BYTES=5`.
...
Example: a velocity vector `[12.5, 3.2, -1.0]` with sub_count=3:
```
03 | 00 00 00 00 00 00 29 40 | 9A 99 99 99 99 99 09 40 | 00 00 00 00 00 00 F0 BF
```

### 4.3 DELTA Encoding (`codec = 0x01`)

Applies to `TYPE_U64`, `TYPE_I32`, and `TYPE_BYTES` (with sub-count > 0) **only**.
Floats (`TYPE_F32`, `TYPE_F64`) fall back to PLAIN — the current implementation
always uses `CODEC_PLAIN` for all columns in `flush_row_group()`.
