...
use module_live_telemetry::{
    compute::{context::ReferenceSource, ComputeRegistry},
    compute::{
        items::RealtimeComputeItem, ComputeContext, ComputeError, ComputeResult,
...
fn compute_realtime_item_returns_correct_value() {
    ...
    registry
        .register_calc_realtime(Box::new(TestSpeedItem))
        .unwrap();
